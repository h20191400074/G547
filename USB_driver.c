...
#define pd_VID  0x0408
#define pd_PID  0xa060

#define HDD_VID  0x1058
#define HDD_PID  0x1078

#define mobile_VID  0x2a70
#define mobile_PID  0xf00e
...
static int device_usb_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	int i;
	...
	if((id->idProduct == pd_PID))
	{
		printk(KERN_INFO "HP pendrive drive detected.\n");
	}
	else if((id->idProduct == HDD_PID))
	{
		printk(KERN_INFO "Western Digital HDD drive detected.\n");
	}
	else if((id->idProduct == mobile_PID))
	{
		printk(KERN_INFO "Mobile phone detected.\n");
	}
	else{
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("M KUSHAGRA <h20191400074@hyderabad.bits-pilani.ac.in> ");
